...
(function(){
    ...
    window.hippify = function() {
        ...
        swap = function(img) {
            ...
            if(imgSrc.indexOf('@2x') === -1 ) url = imgSrc.replace(/(.jpg|.jpeg|.png|.gif|.bmp)/, '@2x$1');
            ajaxTest(url,function() { img.src = url; });
            return;
        };

        swapData = function(img, url) {
            ajaxTest(url,function() { img.src = url; });
            return;
        };
        ...
        if(isHippi && !isMobile) {
            while(imgArr.length) {
                var i = imgArr.pop();
                if(dataUrl = i.getAttribute('data-hippi-src')) swapData(i, dataUrl);
