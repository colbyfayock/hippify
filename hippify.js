...
(function(){
    ...
    var hippify = function() {
        ...
        var isMobile, devicePixelRatio, isHippi, swap, swapData, imgArr,ajaxTest;
        ...
        swap = function(img) {
            var url = img.src.replace(/(.jpg|.jpeg|.png|.gif|.bmp)/, '@2x$1');
            ajaxTest(url,function() { img.src = url; });
            return;
        };

        swapData = function(url) {
            ajaxTest(url,function() { i.src = url; });
            return;
        }
        ...
        if(isHippi && !isMobile) {
            while(imgArr.length) {
                var i = imgArr.pop();
                if(dataUrl = i.getAttribute('data-hippi-src') || false) swapData(dataUrl);
                if(i.getAttribute('data-hippi')) swap(i);
            }
        }
