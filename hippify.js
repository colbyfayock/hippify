...
(function(){
    ...
    var hippify = function() {
        ...
        var isMobile, devicePixelRatio, isHippi, swap, imgArr;
        ...
        swap = function(img) {
            var xmlHttpReq = false,
                that = this,
                url = img.src.replace(/(.jpg|.jpeg|.png|.gif|.bmp)/, '@2x$1');

            if (!window.XMLHttpRequest) return;
            that.xmlHttpReq = new XMLHttpRequest();

            that.xmlHttpReq.open('HEAD', img.src, true);
            that.xmlHttpReq.onreadystatechange = function(e) {
                if (e.target.readyState < 4) return;
                if (e.target.status == 200) img.src = url;
            };
            that.xmlHttpReq.send();
        };

        if(isHippi && !isMobile) {
            for(var i = 0; i < imgArr.length; i++) {
                if(imgArr[i].getAttribute('data-hippi')) swap(imgArr[i]);
            }
        }
    };
